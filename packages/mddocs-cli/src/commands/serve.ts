...
export function registerServe(program: Command): void {
  program.command('serve <file>')
    ...
    .option('--agent <name>', 'register a named agent with its own token (repeatable)', (v, acc: string[]) => [...acc, v], [])
    .action(async (file: string, o: { port?: number; host?: string; autocommit?: boolean; agent?: string[] }) => {
      ...
      const agents = (o.agent ?? []).map((name) => ({ name }))
      const handle = await serveShare(file, {
        port: o.port,
        host: o.host,
        autocommit,
      ...
      if (handle.agentTokens) {
        console.log('    tokens (send as header: x-share-token):')
        for (const [name, token] of Object.entries(handle.agentTokens)) {
